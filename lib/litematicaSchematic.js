...
function read(nbt, version, fallbackBlock="stone") {
    ...
    for (const region_name of Object.keys(nbt.Regions)) {
        ...
        console.log(blockPalette.length);
        ...
        for (var i = 0; i < regionVolume; i++) {
            ...
            //TODO : add Modulo to calculation here if Num > blockPalette.length
            if (start_index === end_index) {
                var index_value = Number(blockStates[Number(start_index)] >> start_bit_offset & bigint_mask);
                //regionBlocks[i] = Number(blockStates[Number(start_index)] >> start_bit_offset & bigint_mask);
            } else {
                ...
                var index_value = Number(value & bigint_mask);
                //regionBlocks[i] = Number(value & bigint_mask);
            }

            if (index_value > region_palette.length) {
                //index_value = (region_palette.length % index_value) - 1;
                index_value = (index_value % region_palette.length) - 1;
            }

            regionBlocks[i] = index_value;
        }
